...
from myUtils import lrn_same_map
#from pylearn2.sandbox.cuda_convnet.response_norm import CrossMapNorm
#from theano.sandbox.cuda.basic_ops import gpu_contiguous

def cifar_fast_net(batch_size=128,n_epochs=500,test_frequency=13, learning_rate=0.001):
...
####define the layers:
    ...
    conv_pool2_input=lrn_same_map(conv_pool1.output,3,0.75,0.00005,16)
    conv_pool2 = LeNetConvPoolLayer(rng=rng2,input=conv_pool2_input,
                                    ...
                                    momB=0.9,
                                    wc=0.004)
    #cross_map_norm_op = CrossMapNorm(size_f=3,add_scale=0.00005,pow_scale=0.75,blocked=False)
    #conv_pool2_output = gpu_contiguous(conv_pool2.output)
    #conv_pool3_input = cross_map_norm_op(conv_pool2.output)
    conv_pool3_input = lrn_same_map(conv_pool2.output,3,0.75,0.00005,8)
    ...
    fc_10 = LogisticRegression(input=layer4_input,rng=rng5,n_in=64*4*4,n_out=10,initW=0.01,
                               epsW=0.001,
                                epsB=0.002,
                                momW=0.9,
                                momB=0.9,
...
####build the models:
    ...
    Ws = [conv_pool1.W, conv_pool2.W, conv_pool3.W, fc_10.W]
    pgradWs = [conv_pool1.grad_W, conv_pool2.grad_W, conv_pool3.grad_W, fc_10.grad_W]

    bs = [conv_pool1.b, conv_pool2.b, conv_pool3.b,  fc_10.b]
    pgradbs = [conv_pool1.grad_b, conv_pool2.grad_b, conv_pool3.grad_b,  fc_10.grad_b]

    momWs = [conv_pool1.momW, conv_pool2.momW, conv_pool3.momW, fc_10.momW]
    momBs = [conv_pool1.momB, conv_pool2.momB, conv_pool3.momB,  fc_10.momB]
    wcs = [conv_pool1.wc, conv_pool2.wc, conv_pool3.wc, fc_10.wc]
    epsWs = [conv_pool1.epsW, conv_pool2.epsW, conv_pool3.epsW,  fc_10.epsW]
    epsBs = [conv_pool1.epsB, conv_pool2.epsB, conv_pool3.epsB, fc_10.epsB]
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    conv_pool2.bestB,conv_pool3.bestW,conv_pool3.bestB,
