...
from layers import LogisticRegression,HiddenLayer,LeNetConvPoolLayer,SaliencyConvLayer,HiddenLayer_dropout
...
####define the layers:
    ...
    fc_64_dropout = HiddenLayer_dropout(rng=rng4,input=layer4_input,n_in=64*4*4,n_out=64,initW=0.1,initB=0,
                        epsW=0.001,
                        epsB=0.002,
                        momW=0.9,
                        momB=0.9,
    ...
    fc_10_dropout = LogisticRegression(input=fc_64_dropout.output,rng=rng5,n_in=64,n_out=10,initW=0.1,
                               epsW=0.001,
                                epsB=0.002,
                                momW=0.9,
                                momB=0.9,
...
####build the models:
    ...
    for W_i, gradW_i, momW_i, wc_i, epsW_i, pgW_i in zip(Ws,gradWs,momWs,wcs, epsWs,pgradWs):
        ...
        updates_init.append((W_i, W_i+grad_i))
        updates_init.append((pgW_i,grad_i))


    for b_i, gradb_i, momB_i, epsB_i, pgB_i in zip(bs,gradbs,momBs, epsBs,pgradbs):
        grad_i = - epsB_i*gradb_i + momB_i*pgB_i
        updates_init.append((b_i, b_i+grad_i))
        updates_init.append((pgB_i,grad_i))
    ...
    train_model_init = theano.function([index], cost, updates=updates_init,
          givens={
            x: train_set_x[index * batch_size: (index + 1) * batch_size],
            y: train_set_y[index * batch_size: (index + 1) * batch_size]})
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        """
        if epoch == 50:
            print 'changing to train the whole model'
            train = train_model
