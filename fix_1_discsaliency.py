...
from layers import LogisticRegression,HiddenLayer,LeNetConvPoolLayer,Fix_1_discsaliency_layer
...
def train_cifar(learning_rate_base=0.01,batch_size=128,n_epochs=100,test_frequency=1300, check_point_frequency=5000,show_progress_frequency=100):
    ...
    if options.filename == 'None':
        ...
        conv_pool1 = Fix_1_discsaliency_layer(rng=rng1,input=img_input,
                                    filter_shape=(3,5,5,32),
                                    image_shape=(3,32,32,batch_size),
                                    activation='relu',
                                    poolsize=(3,3),poolstride=2,pad=2,
                                    convstride=1,alphas=alphas,partial_sum=4,
                                    pooling='max',
                                    name='fix1'
                                    )
    ...
    all_layers = [conv_pool2,conv_pool3,fc_1,fc_2]
#######test model
